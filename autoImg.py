...
class QnewsAutoImg(AutoImg):
    ...
    def assembleFeedsBigAd(self):
        blank_height = self.cf.getint('Qnews', 'feeds_big_blank_height')
        ad_width = self.cf.getint('Qnews', 'feeds_big_ad_width')
        ad_height = self.cf.getint('Qnews', 'feeds_big_ad_height')
        word_height = self.cf.getint('Qnews', 'word_height')
        blank = cv2.imread(self.cf.get('Qnews', 'img_blank'))

        doc_1stline_max_len = self.set1stDocLen(self.doc, 'Qnews')
        # set ad backgroud
        if len(self.doc) <= doc_1stline_max_len:
            bkg = cv2.resize(blank, (self.screen_width, blank_height))
        else:
        ...
        # Add bottom
        _, bottom_height = self.getImgWH(self.cf.get('Qnews', 'img_ad_area_bottom'))
        bkg[blank_height-bottom_height:blank_height, 0:self.screen_width] \
            = cv2.imread(self.cf.get('Qnews', 'img_ad_area_bottom'))
        ...
        draw = ImageDraw.Draw(im)
        if '' != self.doc:
            ttfont = ImageFont.truetype("font/UbuntuDroid.ttf", self.cf.getint('Qnews', 'doc_size'))
            ad_doc_pos = (self.cf.getint('Qnews', 'feeds_big_doc_x'), self.cf.getint('Qnews', 'feeds_big_doc_y'))
            ad_doc_color = self.parseArrStr(self.cf.get('Qnews', 'doc_color'), ',')
            if len(self.doc) <= doc_1stline_max_len:
                draw.text(ad_doc_pos, self.doc, fill=(ad_doc_color[0], ad_doc_color[1], ad_doc_color[2]), font=ttfont)
            else:
                ad_doc_pos1 = (ad_doc_pos[0], ad_doc_pos[1] + word_height)
                draw.text(ad_doc_pos, self.doc[:doc_1stline_max_len], fill=(ad_doc_color[0], ad_doc_color[1], ad_doc_color[2]),
                          font=ttfont)
                draw.text(ad_doc_pos1, self.doc[doc_1stline_max_len:], fill=(ad_doc_color[0], ad_doc_color[1], ad_doc_color[2]),
                          font=ttfont)

        im.save('tmp_img/tmp.png')
        return cv2.imread('tmp_img/tmp.png')

    def set1stFeedsSmallDocLen(self, doc, sec):
        cl = self.cf.getint(sec, 'doc_Chinese_width')
        el = self.cf.getint(sec, 'doc_English_width')
        fl = self.cf.getint(sec, 'feeds_small_doc_1stline_px_len')
        mlen = 0
        for i in range(len(doc)):
            if doc[i] <= '\u2000':
                mlen += el
    ...
    def assembleFeedsSmallAd(self):
        blank_height = self.cf.getint('Qnews', 'feeds_small_blank_height')
        ad_width = self.cf.getint('Qnews', 'feeds_small_ad_width')
        ad_height = self.cf.getint('Qnews', 'feeds_small_ad_height')
        word_height = self.cf.getint('Qnews', 'word_height')
        blank = cv2.imread(self.cf.get('Qnews', 'img_blank'))
        ad_left_x = self.cf.getint('Qnews', 'feeds_small_ad_x')
        ad_top_y = self.cf.getint('Qnews', 'feeds_small_ad_y')

        doc_1stline_max_len = self.set1stFeedsSmallDocLen(self.doc, 'Qnews')
        # set ad backgroud
        bkg = cv2.resize(blank, (self.screen_width, blank_height))

        # Add bottom
        _, bottom_height = self.getImgWH(self.cf.get('Qnews', 'img_feeds_small_bottom'))
        bkg[blank_height - bottom_height:blank_height, 0:self.screen_width] \
            = cv2.imread(self.cf.get('Qnews', 'img_feeds_small_bottom'))
        ...
        if '' != self.doc:
            ttfont = ImageFont.truetype("font/UbuntuDroid.ttf", self.cf.getint('Qnews', 'doc_size'))
            ad_doc_pos = (self.cf.getint('Qnews', 'feeds_small_doc_x'), self.cf.getint('Qnews', 'feeds_small_doc_y'))
            ad_doc_color = self.parseArrStr(self.cf.get('Qnews', 'doc_color'), ',')
            if len(self.doc) <= doc_1stline_max_len:
                draw.text(ad_doc_pos, self.doc, fill=(ad_doc_color[0], ad_doc_color[1], ad_doc_color[2]), font=ttfont)
            else:
    ...
    def assembleFeedsMultiAd(self):
        blank_height = self.cf.getint('Qnews', 'feeds_multi_blank_height')
        ad_width = self.cf.getint('Qnews', 'feeds_multi_ad_width')
        one_ad_width = self.cf.getint('Qnews', 'feeds_multi_1_ad_width')
        ad_height = self.cf.getint('Qnews', 'feeds_multi_ad_height')
        word_height = self.cf.getint('Qnews', 'word_height')
        blank = cv2.imread(self.cf.get('Qnews', 'img_blank'))
        ...
        bkg[ad_top_y:ad_top_y + ad_height, ad_left_x+one_ad_width+ad_space_between:ad_left_x+2*one_ad_width+ad_space_between] \
            = cv2.resize(cv2.imread(img_ads[1]), (one_ad_width, ad_height))
        bkg[ad_top_y:ad_top_y+ad_height, ad_left_x+2*one_ad_width+2*ad_space_between:ad_left_x+3*one_ad_width+2*ad_space_between] \
        ...
        if '' != self.doc:
            ttfont = ImageFont.truetype("font/UbuntuDroid.ttf", self.cf.getint('Qnews', 'doc_size'))
            ad_doc_pos = (self.cf.getint('Qnews', 'feeds_multi_doc_x'), self.cf.getint('Qnews', 'feeds_multi_doc_y'))
            ad_doc_color = self.parseArrStr(self.cf.get('Qnews', 'doc_color'), ',')
            if len(self.doc) <= doc_1stline_max_len:
                draw.text(ad_doc_pos, self.doc, fill=(ad_doc_color[0], ad_doc_color[1], ad_doc_color[2]), font=ttfont)
            else:
    ...
    def getFeedsBlankHeight(self):
        ...
        blank_height = self.cf.getint('Qnews', key)
        if len(self.doc) > self.set1stDocLen(self.doc, 'Qnews') and "feeds_small" != self.ad_type:
            blank_height = blank_height + self.cf.getint('Qnews', 'word_height')
